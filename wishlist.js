...
class Wishlist {
  constructor(opts = {}) {
    ...
    this.countEl = opts.countEl || document.getElementById('wishlistCount');
    this.updateCount();
  }
  ...
  toggle(product){
    const idx = this.items.findIndex(i => i.name === product.name);
    if (idx >= 0) { this.items.splice(idx, 1); } else { this.items.push(product); }
    this.save(); this.updateCount(); this.render();
  }
  ...
  render(){
    ...
    const content = modal.querySelector('.wishlist-content');
    content.innerHTML = '';

    if (this.items.length === 0) {
      content.innerHTML = '<p class="empty-cart">Your wishlist is empty</p>';
      return;
    }
    ...
    this.items.forEach(item => {
      ...
      // ✅ CHANGED: use shared cart; show cart and render for instant feedback
      row.querySelector('.wishlist-add').addEventListener('click', () => {
        ...
        if (cartModal) { cartModal.style.display = 'block'; cart.render(); }
        if (typeof window.toast === 'function') window.toast(`Added ${item.name} to cart!`, 'success');
      });
